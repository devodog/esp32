...
const byte heaterControlOutPin = 1;
const byte highWaterInPin = 15;
const byte lowWaterInPin = 16;
const byte waterValveControlOutPin = 0;
volatile byte valveState = CLOSED;

IRAM_ATTR void valveClose() {
   digitalWrite(waterValveControlOutPin, LOW);
   valveState = CLOSED;
}

IRAM_ATTR void valveOpen() {
   digitalWrite(waterValveControlOutPin, HIGH);
   valveState = OPEN;
}

void setup() {
  pinMode(heaterControlOutPin, OUTPUT);
  pinMode(highWaterInPin, INPUT);
  pinMode(lowWaterInPin, INPUT);
  pinMode(waterValveControlOutPin, OUTPUT);

  attachInterrupt(digitalPinToInterrupt(highWaterInPin), valveClose, RISING);
  attachInterrupt(digitalPinToInterrupt(lowWaterInPin), valveOpen, FALLING | LOW);
